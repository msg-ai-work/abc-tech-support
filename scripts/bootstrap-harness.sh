...
for p in "$DEST/HARNESS.md" "$DEST/.kiro/steering" "$DEST/.kiro/skills" ".kiro/agents/enterprise-messaging-tech-support.json"; do [[ -e "$p" ]] || { echo "Required Harness resource missing: $p" >&2; exit 1; }; done
echo "Common Harness ready: $REPO @ $REF"
echo 'Kiro Agent: enterprise-messaging-tech-support'
